...
主菜单的“启动方式”包含假回锁、真实状态和备份三个入口。手动选择后，需要在确认页按电源键确认三次，任一步按音量键即可取消；取消不会保存默认项或启动。假回锁和真实状态在确认后保存为默认，备份不覆盖默认。无人值守启动仍使用原有默认记录，不弹出确认页。

BL 锁定、解锁以及 Critical 状态修改也需要三次确认；ARB 保持五次确认。每步确认间隔一秒并清理输入缓冲，实体按键长按行为仍需在对应设备上验证。升级时需一并更新 BDS、工具和 `efisp` 菜单配置（含新增的 `BOOTMODES`），建议使用完整新版发行包。

### 二级菜单汉化

Fastboot 的关机/重启菜单与重启工具、BL 锁状态工具、防回滚工具现使用同一黑白中文界面，涵盖按键提示、状态、错误和操作确认。BL 写入前三次确认，ARB 五次确认，音量键取消。无可用 GOP 时明确提示并保留英文控制台。

升级这部分界面需要同时更新 `BDS.efi` 与 `efisp/tools/` 下的三个 EFI 工具；只更新 BDS 不会改变旧工具的界面。

### 开机提示开关

BDS 的“安卓工具”子菜单提供“开机提示：开／关”，默认关闭，按电源键切换并保存，重启后保留选择。该设置控制一加／OPlus 的原厂解锁提示，适用于随包的假回锁、真实状态及备份启动入口，手动启动和默认启动均生效。

关闭时保持当前启动文件的隐藏行为；开启时，在真实 BL 已解锁且启动文件包含本项目已识别的去黄字补丁时，恢复提示的显示路径。修改仅发生在加载镜像的内存副本中，不改变磁盘上的启动文件或真实锁状态。真实 BL 已锁定时不会强制显示解锁提示。无需重新修补 ABL，更新 `BDS.efi` 即可使用；不支持的镜像、设置读取或保存失败会明确报错，不会假装生效。

设置沿用 `efisp` 尾部存储，保留旧默认项和自定义项的原有位置。仅适用于菜单加载的随包安卓入口，不控制进入 BDS 之前由原始固件显示的提示。真机按键、显示效果及固件加载兼容性仍需在对应设备上验证。
